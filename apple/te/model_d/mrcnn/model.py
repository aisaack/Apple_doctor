...
from te.model_d.mrcnn import utils
...
# keras.backend.tensorflow_backend.set_session(tf.Session(config=config_tf))
    ...
    while True:
        try:
            ...
            if not np.any(gt_class_ids > 0):
                continue
            #print("gt_masks:",gt_masks.shape, "gt_masks[0].sum", np.sum(gt_masks[0]) )
            if not np.any(gt_masks):
                continue
            # RPN Targets
